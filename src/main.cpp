		...
		while (!file.eof())
		{
			...
			if (temp.compare(2, 7, "General") == 0)
			{
				...
				if (debug == 1)
				{
					...
					cout << "mineral_count: " << mineral_count << endl;
					cout << "vespene_gas_count: " << vespene_gas_count << endl;
					cout << "worker: " << worker << endl;
					cout << "BaseBuilding: " << basebuilding << endl;
				}
	...
	if (file.good())
	{
		while (!file.eof())
		{
			...
			else if (temp.compare(2, 4, "Zerg") == 0 && race.compare("sc2-hots-zerg") == 0)
			{
				...
				if (debug == 1)
				{
					cout << "ConfigFile Zerg erweiterung" << endl;
					cout << "Overlord: " << overlord << endl;
					cout << "Larvae: " << larvae << endl;
				}
			}
		}
		file.close();
...
void initialize(const std::string& race) {
	...
	if (race.compare("sc2-hots-terran") == 0)
	{
		TerranGame game_setup(mineral_count, vespene_gas_count); //Terran game erstellen
		for (unsigned int runs = 0;runs < 2;runs++)
		{ 
		file.open(techtreefilename, ios::in);
		if (file.good())
	...
	{
		ProtossGame game_setup(mineral_count, vespene_gas_count); //Terran game erstellen
		for (unsigned int runs = 0;runs < 2;runs++)
		{
			file.open(techtreefilename, ios::in);
			if (file.good())
	...
	{
		ZergGame game_setup(mineral_count, vespene_gas_count); //Terran game erstellen
		for (unsigned int runs = 0;runs < 2;runs++)
		{
			file.open(techtreefilename, ios::in);
			if (file.good())
		...
		for (unsigned int b = 0;b < overlord;b++)
		{
			game_setup.add_building_by_name("overlord");
		}
		for (unsigned int c = 0;c < larvae;c++)
		{
			game_setup.add_building_by_name("larva");
		}
		//return game_setup;
	}
	else
